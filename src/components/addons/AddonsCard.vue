...
<template>
    <div class="relative rounded-lg overflow-hidden shadow-xl group h-50">
        ...
        <div class="absolute inset-0 flex flex-col justify-end p-4">
            <h3 class="text-white font-semibold text-lg drop-shadow-md">{{ addon.title }}</h3>
            <div class="mt-2 flex items-center gap-3">
                ...
                <span class="ml-auto text-xs text-white/90 bg-black/25 px-2 py-1 rounded">
                    {{ addon.priceType === 'fixed' ? (addon.priceValue + ' kr') :
                        addon.priceType === 'percentage' ? (addon.priceValue + '%') :
